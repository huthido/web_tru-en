...
- **Garage v1.0.1** (S3-compatible object storage, có fallback Supabase/Cloudinary/local)
...
### ⚠️ Bảo mật
- **Quay password** tất cả tài khoản cũ (Supabase, Postgres) nếu trước đây từng dùng trong các script test (`test-db*.js` đã bị xoá khỏi repo nhưng đã chạy local).
...
| Resource | Loại | Note |
|---|---|---|
| **Postgres** | Coolify template hoặc managed (Supabase/Neon) | Ghi lại `DATABASE_URL` |
...
**Priority order:** Garage → Supabase → Cloudinary → local `/uploads`
...
| `SUPABASE_URL` | Supabase | Optional, fallback nếu không có Garage |
| `SUPABASE_KEY` | Supabase | |
| `SUPABASE_BUCKET` | Supabase | `web-truyen` |
...
### 7.4 Seed admin user lần đầu

Container backend tự chạy `npx prisma migrate deploy` lúc start, nhưng không tự seed. Tạo admin:
...
Hoặc tạo thủ công qua API `/api/auth/register` rồi update role trong Postgres:
...
- [ ] Quay password tất cả service cũ (Supabase, DB) nếu từng share trong code
