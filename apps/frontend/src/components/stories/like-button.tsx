...
import { ICON_BTN, ICON_BTN_LIKED, BTN_TONAL, BTN_LIKED } from '@/components/ui/action-button-styles';
...
export function LikeButton({ storyId, likeCount: initialLikeCount, className = '', showCount = true }: LikeButtonProps) {
  ...
  if (!user) {
    return (
      <button
        disabled
        className={`${showCount ? BTN_TONAL : ICON_BTN} opacity-50 cursor-not-allowed ${className}`}
      >
  ...
  return (
    <button
      ...
      className={`${
        isLiked
          ? (showCount ? BTN_LIKED : ICON_BTN_LIKED)
          : (showCount ? BTN_TONAL : ICON_BTN)
      } ${className}`}
    >
      {isLoading ? (
        <>
        ...
        <svg
          className={`w-5 h-5 md:w-6 md:h-6 shrink-0 ${isLiked ? 'text-red-500 dark:text-red-400' : ''}`}
          viewBox="0 0 24 24"
